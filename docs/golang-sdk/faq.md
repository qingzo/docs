...
**Windows 10 / 11**（amd64 / 386 / arm64）与 **Linux**（amd64 / arm64）。两侧公共 API 完全一致，同一份业务代码直接跨平台编译。macOS 暂不支持。
...
不需要。Windows 侧是**纯 Go 实现**（syscall 直调），DLL 已 `go:embed` 内置——无需 MinGW / MSYS2，无需设置 `CC` / `CGO_ENABLED`，也无需随程序分发 DLL。

### Linux 上编译报 `stdlib.h: No such file` / `cannot find -lxdo`？

Linux 侧走 cgo，构建机需要装系统开发包：
...
`libxdo-dev` 容易漏——`libJadeView.a` 硬引用它（托盘菜单发按键序列），漏了链接期报 `cannot find -lxdo`。
...
- **Linux**：不用 WebView2，渲染走系统 WebKit2GTK
...
官方 proxy 索引有几分钟延迟。改用精确版本号（如 `@v0.2.2`），或加 `GOPROXY=https://proxy.golang.org,direct` 显式拉取。
...
- `FrameStyle.TitleOverlay` —— 保留边框、无标题栏、库内置右上角控制按钮（Windows，推荐）
...
`jade-region-drag` 拖动区是 Windows 特性；Linux 用 CSS `-webkit-app-region: drag`。

---

## 平台相关

### Linux 上调用 TrayCreate 就崩溃？

上游 beta.10 已知问题：桌面**没有 StatusNotifier 托盘协议**时（如 Debian/GNOME 默认桌面），`tray_create` 不是返回 0 而是让 GUI 线程直接崩溃。调用前先用 D-Bus 探测 `org.kde.StatusNotifierWatcher`，无支持则跳过托盘。探测代码见[高级用法 - Linux 托盘协议探测](./advanced#linux-托盘协议探测)。GNOME 用户可安装 AppIndicator 扩展获得托盘支持。

### Linux 虚拟机 / 无 GPU 环境启动就崩（libEGL 报错）？

WebKit 默认走 GPU 合成，拿不到 EGL/DRI 时直接崩溃。强制软件渲染：
...
VMware 虚拟机开启「3D 加速」也可解决。

### SSH / su 到 root 后运行报 No authorisation provided？

X/Wayland 授权 cookie 属于登录用户，`su`/`sudo` 切用户后丢失。请用**原登录用户**运行 GUI 程序。
